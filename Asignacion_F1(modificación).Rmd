...
```{r}
par(mfrow = c(2,3))
scatter.smooth(edad, log(pr/(1-pr)), cex = 0.5) #Esta variable parece no tener una linealidad
...
ggplot(data = tips2) +
  ...
  geom_line(aes(group = cat_cloA))## Lo datos no tienen  un comportamiento lineal,al revisar la grafica se requiere revisar la variable que no tiene el comportamiento lineal
```
...
dfa2 <- rnorm_multi(n = 1000,
            mu = c(67, 30, 30, 320),
            sd = c(2, 3, 5, 10),
            varnames = c('edad', 'dapf', 'rto', 'clolA'),
            r = c(0.4, 0.6, 0.5, 0.6, 0.7, 0.8))

dfa2$hibrido <- round(runif(n = 1000, min = 0,max = 1.2),)

w <- 0.5 * dfa2$clolA - 0.01 * dfa2$dapf - 0.6 * dfa2$rto - 0.02 * dfa2$edad

dfa2$abortos <- ifelse(w > 140, 'Si_aborto', 'No_aborto')
...
```{r}
univariable_edad <- glm(abortos ~ edad, family = binomial, data = dfa2)
class(abortos)
summary(univariable_edad)
#(ERROR)
summary(model1_2)
```
```{r}
model2_2 <- glm(abortos ~ edad + dapf + rto + clolA, family = binomial, data = dfa2)#Modelo con la variable Eliminada.
summary(model2_2)
...
```{r}
model3_2 <- glm(abortos ~ edad + dapf + rto, family = binomial, data = dfa2) # Elimiando la variable con el p  value más alto
summary(model3_2)
```
```{r}
...
model_final_2 <- glm(abortos ~ edad + dapf + rto + clolA, family = binomial, data = dfa2)#Modelo con la variable Eliminada.
summary(model_final_2)
```
```{r}

model_final_3 <- glm(abortos ~ edad + dapf + rto, family = binomial, data = dfa2)#Modelo con la variable Eliminada.
summary(model_final_3)
```

```{r}

model_final_4 <- glm(abortos ~ dapf + rto, family = binomial, data = dfa2)#Modelo con la variable Eliminada.
summary(model_final_4)
```
```{r}

model_final_5 <- glm(abortos ~rto, family = binomial, data = dfa2)#Modelo con la variable Eliminada.
...
lrtest(model_final_5,model_final_4)
...
par(mfrow = c(2,3))
scatter.smooth(edad, log(pr/(1-pr)), cex = 0.5) # Es una pendiente muy horizontal

scatter.smooth(dapf, log(pr/(1-pr)), cex = 0.5)#No Corre el objeto(ERROR)

scatter.smooth(rto, log(pr/(1-pr)), cex = 0.5)#Tiene una pendiente muy horizontal
...
model_final_6 <- glm(abortos ~ dap, family = binomial, data = dfa2)# revisar un modelo con la variable lineal pero con mala probabilidad. 
summary(model_final_6)
```

```{r}

model_final_f <- glm(abortos ~ dap, family = binomial, data = dfa2)# Utilizar el modelo y revisar en las conclusiones finales su capacidad de predecir aborto
